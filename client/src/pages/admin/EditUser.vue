...
export default {
  ...
  mounted(){
  	...
  	// 	alert(e)
  	// }),
      this.axios.post("http://localhost:8083/user/allUser",{
      })
        .then(respose => {
  ...
  methods:{
  	deleteUser(id){
      this.axios.post("http://localhost:8083/user/deleteUser",{
        id:id,
      })
        .then(respose => {
          console.log(respose.data);
  	...
  	searchUser(){
  		const temp = this.$refs.input.value;
  		// const res = getSearchUser(val);
      this.axios.post("http://localhost:8083/user/searchUser",{
        temp:temp,
      })
        .then(respose => {
          console.log(respose.data);
